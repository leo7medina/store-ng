...
export class ProductService {
  ...
  getOne(params: {product_id?: string; product_slug?: string}) {
    if (!params.product_id && !params.product_slug) {
      throw new Error('Se requiere al menos un parámetro: product_id o product_slug');
    }
    const endpoint = params.product_id ? `products/${params.product_id}` : `products/slug/${params.product_slug}`;
